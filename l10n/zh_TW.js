OC.L10N.register(
    ...
    "Multi-select" : "多重選擇",
    "User inputs" : "使用者輸入",
    "Specify any number of input values which the user may provide when running this action. These will be accessible in the script via the get_input() function." : "指定使用者在執行此動作時可以提供的任意數量的輸入值。指令稿可以透過 get_input() 函式存取這些值。",
    ...
    "File actions are small Lua scripts that can create, modify, and/or delete files programmatically. These actions may be triggered by users to run on their files. Please read the documentation for more information." : "檔案動作是可以程式方式建立、修改及／或刪除檔案。使用者可能會觸發這些動作以在其檔案上執行。請閱讀文件以取得更多資訊。",
